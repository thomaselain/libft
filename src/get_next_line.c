...
/*   Updated: 2016/03/05 19:28:13 by telain           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
int			sub_cpy(t_gnl *gnl, char **line, int ret)
{
	if (ret == 0)
	{
		*line = ft_strsub(gnl->str, gnl->s, find_bn(gnl->str, gnl->s, 0));
		gnl->end = 1;
		gnl->s = 0;
		return (0);
	}
	...
	{
		*line = ft_strsub(gnl->str, gnl->s, find_len(gnl->str, gnl->s));
...
int			get_next_line(int const fd, char **line)
{
	...
	while (find_bn(gnl->str, gnl->s, 1) == -1 && ret > 0)
	{
